      ...
      <div className="mx-auto max-w-[1400px] px-4 md:px-6 py-4 md:py-6 flex gap-6">
          ...
          <nav className="space-y-4 overflow-y-auto pr-1">
            ...
            {currentTab?.sections.map(s => (
              <div key={s.section} className="space-y-1">
                ...
                <ul className="pl-4 list-disc text-sm text-slate-600 dark:text-slate-300">
                  {s.categories.map(c => (
                    <li key={`${s.section}-${c.category}`}>
                      <button
                        className="hover:underline"
                        onClick={() => {
                          const el = document.getElementById(`cat-${s.section}::${c.category}`)
                          if (el) el.scrollIntoView({ behavior: 'smooth', block: 'start' })
                        }}
                      >
                        {c.category}
        ...
        <main className="flex-1 md:ml-72 space-y-6">
          ...
          {!isSearching && !showFav && !showCustom && currentTab && currentTab.sections.map(s => (
            <section key={s.section} id={`sec-${s.section}`} className="space-y-3">
              <h2 className="text-2xl font-semibold text-ink dark:text-white">{s.section}</h2>
              {s.categories.map(c => (
                <div key={`${s.section}-${c.category}`} id={`cat-${s.section}::${c.category}`} className="space-y-2">
                  <h3 className="text-lg font-semibold text-ink dark:text-white">{c.category}</h3>
                  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {c.prompts.map((p) => {
